...
// ---
var WSChannelContext = function( conn, req )
{
    request.ChannelContext.call( this );
    this._ws_conn = conn;
    this._ws_req = req;
...
var NodExecutor = function( ccm, opts )
{
...
_.extend( NodExecutor, NodeExecutorConst );

var NodExecutor_proto = new executor.Executor();
_.extend( NodExecutor_proto, NodeExecutorConst );
_.extend( NodExecutor_proto, events.EventEmitter.prototype );
NodExecutor.prototype = NodExecutor_proto;

NodExecutor_proto._http_server = null;
NodExecutor_proto._http_path = null;
NodExecutor_proto._ws = null;

NodExecutor_proto.handleHTTPRequest = function( req, rsp )
{
    var _this = this;
            ...
            ( req_url + '/' === http_path ) )
    {
        ...
        req.on(
            ...
            function()
            {
                _this.emit( 'error', req, 'Invalid Request' );
            }
        );
    ...
    var ftnreq =
    {
        f : pathname.slice( 1, 3 ).join( ':' ),
        p : parsed_url.query
    };

    if ( pathname.length > 4 )
    {
        ftnreq.sec = pathname[4];
    }

    var reqinfo = new request.RequestInfo( _this, req );
    this._handleHTTPRequestCommon( reqinfo, req, rsp, ( request.method === 'POST' ) );
};

NodExecutor_proto._handleHTTPRequestCommon = function( reqinfo, req, rsp, raw_upload )
{
...
NodExecutor_proto.handleWSConnection = function( ws )
{
    var _this = this;
...
NodExecutor_proto._handleWSRequest = function( context, req )
{
    var reqinfo = new request.RequestInfo( _this, req );
    ...
    as.add(
        ...
        function( as, err )
        {
            void as;
            void err;
            context._ws_conn.send( '{"e":"InternalError"}' );
        }
    ).add(
        function( as )
        {
...
exports = module.exports = NodExecutor;
