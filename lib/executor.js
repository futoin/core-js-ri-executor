...
executor.prototype =
{
    ...
    register : function( as, ifacever, impl )
    {
        ...
        this._ifaces[ iface ][ mjr ] = info;
        this._impls[ iface ][ mjr ] = impl;

        for ( var i = 0; i < info.inherits.length; ++i )
        {
            var supm = ifacever.match( invoker.SpecTools._ifacever_pattern );
            var supiface = supm[1];
        ...
        as.add(
            function( as )
            {
                ...
                as.add( function( as )
                {
                    ...
                    var impl = this._getImpl( as, reqinfo );
                ...
                as.add( function( as, result )
                {
                    ...
                    this._checkResult( as, reqinfo );

                    this._signResponse( as, reqinfo );
                    this._packResponse( as, reqinfo );
                } );
            },
            function( as, err )
            {
    ...
    _getInfo : function( as, reqinfo )
    {
        ...
        var f = reqinfo_info[reqinfo.INFO_RAW_REQUEST];
    ...
    {
        var rawreq = reqinfo[ reqinfo.INFO_RAW_REQUEST ];
        ...
        if ( 'p' in rawreq )
        {
            ...
            for ( k in finfo.params )
            {
                if ( !( k in reqparams ) )
                {
                    ...
                    if ( 'default' in pinfo.default )
                    {
                        reqparams[k] = pinfo.default;
                    }
                    else
                    {
                        as.error( FutoInError.InvalidRequest, "Missing parameter" );
                    }
                }
            }
        }
    ...
    {
        var rsp = reqinfo[ reqinfo.INFO_RAW_RESPONSE ];
        var finfo = as.state._futoin_func_info;
    ...
    {
        if ( !reqinfo[ reqinfo.INFO_DERIVED_KEY ] )
        {
            return;
        }
